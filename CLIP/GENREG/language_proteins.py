...
class CategoryProtein(Protein):
    ...
    def forward(self, signals, protein_outputs):
        ...
        Expects signal 'category_match' with value:
            ...
            0.0 = unrelated
        """
        # Read from bound input if available, otherwise fall back to self.name
        signal_name = self.inputs[0] if self.inputs else self.name
        match_level = signals.get(signal_name, protein_outputs.get(signal_name, 0.0))
        ...
        if match_level >= 0.9:
            ...
            # Unrelated - PENALIZE wrong predictions
            self.output = -self.params.get("mismatch_penalty", 0.0)
...
class ProximityRewardProtein(Protein):
    ...
    def forward(self, signals, protein_outputs):
        ...
        Uses exponential decay with penalty baseline:
        reward = (max * exp(-steepness * distance / half_life)) - penalty_baseline
        """
        ...
        # Subtract penalty baseline to make far predictions negative
        penalty_baseline = self.params.get("penalty_baseline", 0.0)
        reward = (self.params["max_reward"] * decay) - penalty_baseline

        # Apply minimum floor
        min_reward = self.params.get("min_reward", -2.0)
        self.output = max(min_reward, reward)
